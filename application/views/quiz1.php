...
<script>
	...
	let currentQuestion = -1; // Start with -1 to handle the initial question
	let numQuestions = 0;
	...
	function loadQuestion() {
		if (currentQuestion === -1) {
			...
			if (incorrectAnswers.length > 0) {
				displayMessage(`Questions Answered Incorrectly: ${incorrectAnswers.join(', ')}`, 'bot');
				// Hit the API to get suggestions and relevant websites
				const requestData = {
					incorrect_answers: incorrectAnswers.map((answer, index) => {
						return {
				...
				$.ajax({
					url: '<?= base_url('index.php/welcome/getSuggestionOfWebsites')?>', // Replace with your API endpoint URL
					type: 'POST',

					data: {
						jsonData: requestData, // Your JSON data here
					},
					success: function(response) {
			...
			// Rest of your code to save statistics and disable the button
	...
	document.getElementById('send-btn').addEventListener('click', () => {
		...
		if (currentQuestion === -1) {
			...
			const correctAnswerIndex = shuffledQuestions[currentQuestion].choices.findIndex(
				(choice) =>
					choice.toLowerCase() === shuffledQuestions[currentQuestion].answer.toLowerCase()
			);
			...
			if (userAnswerIndex === correctAnswerIndex) {
				...
				incorrectAnswers.push(`Question ${currentQuestion + 1}`);
			}
			currentQuestion++;
		}
		loadQuestion();
